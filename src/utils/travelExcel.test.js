...
describe('travelExcel plan workbook', () => {
  ...
  it('parses dual-currency headers including RMB columns', async () => {
    const wb = createWorkbook();
    const trip = wb.addWorksheet('出游信息');
    trip.addRow(['标题*', '目的地', '开始日期', '结束日期', '状态', '人数', '行程备注']);
    ...
    const budget = wb.addWorksheet('预算信息');
    budget.addRow(['出行币', '本币', '汇率', '汇率日期', '备注', '主线']);
    budget.addRow(['JPY', 'CNY', 0.043, '2026-09-07', '', '否']);
    const items = wb.addWorksheet('预算明细');
    items.addRow(['类别', '项目*', '数量', '单价(日元)', '本币单价(人民币)', '小计(日元)', '本币小计(人民币)', '状态', '可选', '主币', '备注']);
    items.addRow(['机票', '往返机票', 1, 155628, 6692, 155628, 6692, '已订', '否', '本币', '']);
    const expenses = wb.addWorksheet('实际记账');
    expenses.addRow(['日期', '项目*', '类别', '金额(日元)', '本币金额(人民币)', '对照预算项目', '备注']);
    expenses.addRow(['2026-09-07', '往返机票', '机票', 155628, 6692, '往返机票', '']);

    const parsed = await parsePlanExcel(await toFile(wb, 'dual.xlsx'));
    expect(parsed.budget.items[0]).toMatchObject({
      amount: 155628,
      amount_cny: 6692,
      quote_in: 'base',
    });
    expect(parsed.budget.expenses[0].amount_cny).toBe(6692);
  });
